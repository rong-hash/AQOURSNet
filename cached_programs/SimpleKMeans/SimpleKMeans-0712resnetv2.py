...
# DATASET
log_filename = 'SimpleKMeans-0712resnet-exp2.csv'
datasets = ['Coffee', 
...
def extract_shapelets(series, shapelet_num, shapelet_len, crop):
    print(int(shapelet_len*crop))
    candidates = np.concatenate([series[:, i:i+shapelet_len] for i in range(0, series.shape[1] - shapelet_len + 1, int(shapelet_len*crop))])
    ...
    for shapelet_num in shapelet_num_range:
        for shapelet_len_prop in shapelet_len_prop_range:
            for crop_len in crop:
                ...
                write_log('%s,%d,%f,%f,' % (dataset, shapelet_num, shapelet_len_prop, crop_len))
