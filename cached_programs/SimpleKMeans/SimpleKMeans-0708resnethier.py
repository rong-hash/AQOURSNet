...
class FCResidualBlock(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.block = nn.Sequential(
                        ...
                        nn.Linear(hidden_size, input_size),  nn.BatchNorm1d(input_size),  nn.ReLU())
    def forward(self, x):
        return self.block(x) + x

class FCResidualBranch(nn.Module):
    def __init__(self, input_size, hidden_sizes):
        super(FCResidualBranch, self).__init__()
        self.blocks = []
        for hidden_size in hidden_sizes:
            self.blocks.append(FCResidualBlock(input_size, hidden_size).to(device))
    def forward(self, x):
...
class NeuralNetwork(nn.Module):
    def __init__(self, input_size, output_size, branch_num, hidden_sizes=[64, 128, 256]):
        super(NeuralNetwork, self).__init__()
        self.branches = []
        for _ in range(branch_num):
            self.branches.append(FCResidualBranch(input_size, hidden_sizes).to(device))
...
# DATASET
log_filename = 'SimpleKMeans-0708resnethier-exp.csv'
datasets = ['FordA']
