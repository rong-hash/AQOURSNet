...
shapelet_len_prop_range = [.15, .25, .35]
crop = [.2, .5, .7, 1]
...
# DATASET
log_filename = 'SimpleKMeans-0713resnet-exp3.csv'
datasets = ['Coffee', 
...
def extract_shapelets(series, shapelet_num, shapelet_len, crop_len):
    candidates = np.concatenate([series[:, i:i+shapelet_len] for i in range(0, series.shape[1] - shapelet_len + 1, int(crop_len*shapelet_len))])
...
write_log('dataset,shapelet_num,shapelet_len,crop_len,train_acc,test_acc\n')
warnings.filterwarnings('ignore')
device = 'cuda' if torch.cuda.is_available() else 'cpu'
seed_torch()


train_data, train_label, valid_data, valid_label, test_data, test_label = get_data('/data/chenzy/stealpower_split.npz')
...
for shapelet_num in shapelet_num_range:
    for shapelet_len_prop in shapelet_len_prop_range:
        for crop_len in crop:
            shapelet_len = int(shapelet_len_prop * train_data.shape[1])
            try:
                shapelets = extract_shapelets(train_data, shapelet_num, shapelet_len, crop_len)
            except:
                continue
            write_log('%s,%d,%d,%d,' % ("stealpower", shapelet_num, shapelet_len, int(shapelet_len*crop_len)))
            train_mat = distance_matrix(train_data, shapelets)
            valid_mat = distance_matrix(valid_data, shapelets)
            ...
            for epoch in range(num_epoch):
                ...
                with torch.no_grad():
                    out = model(torch.Tensor(valid_mat).to(device))
            ...
            print('%-24s\t%d\t%d\t%5.2f%%\t%5.2f%%' \
                % ("stealpower", shapelet_num, shapelet_len, best_train_acc * 100, best_test_acc * 100))
