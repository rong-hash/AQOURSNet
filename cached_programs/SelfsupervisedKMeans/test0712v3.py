...
from SK0712v2 import  extract_shapelets_with_selfsupervised_learning
import matplotlib.pyplot as plt
######## TO BE MODIFIED ######## TO BE MODIFIED ######## TO BE MODIFIED ######## TO BE MODIFIED ########
########################################################################################################

# HYPERPARAMETERS
shapelet_num_range = [40]
shapelet_len_prop_range = [.15]
learning_rate = 1e-3
num_batch = 8
num_epoch = 100
...
# DATASET
log_filename = 'SelfsupervisedKMeans-0712resnet-exp4.csv'
datasets = ['BeetleFly', 
            'Coffee', 
...
def draw_plot(shapelets, dataset):
    plt.figure(figsize=(10, 10))
    for i in range(shapelets.shape[0]):
        plt.plot(shapelets[i])
    plt.show()
    plt.savefig('../shapelets_fig/%s' % dataset)
    return 
...
for dataset in datasets:
    ...
    for shapelet_num in shapelet_num_range:
        for shapelet_len_prop in shapelet_len_prop_range:
            ...
            try:
                shapelets = extract_shapelets_with_selfsupervised_learning(train_data, shapelet_num, shapelet_len)
                draw_plot(shapelets, dataset)
            except:
                continue
