...
def extract_shapelets_with_selfsupervised_learning(train_data, shapelets_num, shapelet_len):
    ...
    device = init_dl_program(3, seed=None, max_threads=None)
    model = TS2Vec(
    ...
    with HiddenPrints():
        dis, classes = kmeans(torch.from_numpy(train_repr), num_clusters=shapelets_num, device=torch.device("cuda:3"))
